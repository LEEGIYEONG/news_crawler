...
def makeCirCularGraph(contents):
    ...
    for i, l in enumerate(labels):
        ...
        if frequency[i]/total*100 < threshold:
            x = (r/2+spacing*count_less_5pct)*np.cos(np.pi /
                                                     # 텍스트 x좌표
                                                     180*((ang1+ang2)/2)) + center[0]
            y = (r/2+spacing*count_less_5pct)*np.sin(np.pi /
                                                     # 텍스트 y좌표
                                                     180*((ang1+ang2)/2)) + center[1]
            count_less_5pct += 1
        else:
        ...
        if i < len(labels) - 1:
            ...
            ax.text(x, y, f'{frequency[i]/total*100:.2f}%',
                    ha='center', va='center', fontsize=15)
        else:  # 마지막 파이 조각은 퍼센티지의 합이 100이 되도록 비율을 조절
            ax.text(x, y, f'{100-sum_pct:.2f}%',
                    ha='center', va='center', fontsize=15)
